...
exports.BedrockAgentValidatorClass = exports.BedrockAgentValidator = void 0;
exports.getBedrockAgentValidator = getBedrockAgentValidator;
const BedrockAgentToolRegistry_1 = require("./BedrockAgentToolRegistry");
/**
 * Validator for Bedrock Agent tool integrations
 * Ensures tools meet AWS Bedrock Agent requirements and work correctly
 */
class BedrockAgentValidator {
    constructor() {
        this.toolRegistry = (0, BedrockAgentToolRegistry_1.getBedrockAgentToolRegistry)();
    }
    /**
     ...
     */
    generateSampleValue(propSchema, key) {
        switch (propSchema.type) {
            case 'string':
                if (key === 'merchant_id')
                    return 'test-merchant-validation';
                if (key === 'user_id')
                    return 'test-user';
                if (key === 'session_id')
                    return 'test-session';
                if (key === 'sku')
                    return 'TEST-SKU-001';
                if (key === 'query')
                    return 'test product search query';
...
Respond with a structured plan for tool invocations.`,
     ...
     */
    async validateBedrockIntegration() {
        const errors = [];
        const warnings = [];
        // Test tool registry
...
exports.BedrockAgentValidator = BedrockAgentValidator;
exports.BedrockAgentValidatorClass = BedrockAgentValidator;
// Singleton instance
let validatorInstance = null;
function getBedrockAgentValidator() {
    if (!validatorInstance) {
        validatorInstance = new BedrockAgentValidator();
    }
    return validatorInstance;
}
//# sourceMappingURL=BedrockAgentValidator.js.map