...
exports.getDriftDetectionService = exports.DriftDetectionService = void 0;
...
class DriftDetectionService {
    constructor() {
        ...
        this.modelRetrainingService = (0, ModelRetrainingService_1.getModelRetrainingService)();
     ...
     */
    stopMonitoring(merchantId, predictorName) {
     ...
     */
    async establishBaseline(config) {
        const cacheKey = `baseline:${config.merchantId}:${config.predictorName}`;
        // Check if baseline already exists
        const existingBaseline = await this.cacheService.get(cacheKey);
        if (existingBaseline) {
    ...
    async checkForDrift(config) {
        ...
        const driftMetrics = {
            ...
            featureImportanceShift: this.calculateFeatureImportanceShift(baseline.featureImportanceBaseline, currentFeatureImportances),
     ...
     */
    async collectRecentPredictions(config, limit) {
     ...
     */
    async calculateAccuracy(config, predictions) {
     ...
     */
    async calculateDataDistribution(config) {
     ...
     */
    async calculateDataDistributionShift(config, baseline) {
     ...
     */
    aggregateFeatureImportances(predictions) {
        const aggregated = {};
        for (const prediction of predictions) {
            if (prediction.featureImportance) {
                for (const [feature, importance] of Object.entries(prediction.featureImportance)) {
                    ...
                    aggregated[feature].push(importance);
                }
            }
        }
        const result = {};
     ...
     */
    calculateFeatureImportanceShift(baseline, current) {
        const shift = {};
     ...
     */
    shouldTriggerRetraining(config, baseline, metrics) {
     ...
     */
    async generateDriftAlerts(config, baseline, metrics) {
        const alerts = [];
     ...
     */
    async triggerAutoRetraining(config, metrics) {
        try {
     ...
     * Store drift metrics for historical analysis
     */
    async storeDriftMetrics(metrics) {
     ...
     */
    async getDriftHistory(merchantId, predictorName, days = 7) {
     ...
     */
    createEmptyDriftMetrics(config) {
        return {
            merchantId: config.merchantId,
            predictorName: config.predictorName,
            timestamp: new Date(),
...
let driftDetectionServiceInstance = null;
const getDriftDetectionService = () => {
...
exports.getDriftDetectionService = getDriftDetectionService;
//# sourceMappingURL=DriftDetectionService.js.map