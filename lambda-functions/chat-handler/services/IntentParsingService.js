...
exports.IntentParsingService = void 0;
exports.createIntentParsingService = createIntentParsingService;
...
class IntentParsingService {
    ...
    async parseIntent(query, context) {
        try {
            ...
            if (parsedIntent.intent === 'question' || parsedIntent.confidence < 0.7) {
                const qContext = await this.amazonQService.getAdditionalContext(query, context.merchantId, 'general');
     ...
     */
    async generateExecutionPlan(intent, context) {
        try {
            const steps = [];
...
`;
     ...
     */
    buildEnhancedIntentPrompt(query, context, qContext) {
        return `
...
`;
     ...
     */
    generateSearchSteps(intent, context) {
        return [
            {
                id: 'semantic_search',
                type: 'retrieval',
     ...
     */
    generateRecommendationSteps(intent, context) {
        return [
            {
                id: 'semantic_search',
                type: 'retrieval',
     ...
     */
    generatePurchaseSteps(intent, context) {
        return [
            {
                id: 'validate_cart',
                type: 'validation',
     ...
     */
    generateQuestionSteps(intent, context) {
        return [
            {
                id: 'knowledge_search',
                type: 'retrieval',
     ...
     */
    generateComparisonSteps(intent, context) {
     ...
     */
    generateSupportSteps(intent, context) {
        return [
            {
                id: 'support_search',
                type: 'retrieval',
     ...
     */
    canParallelizeSteps(steps) {
        // Steps can be parallelized if they don't have dependencies on each other
        const stepIds = new Set(steps.map(s => s.id));
        return steps.every(step => step.dependencies.every(dep => !stepIds.has(dep)));
    }
    /**
     * Generate fallback plan
     */
    generateFallbackPlan(intent, context) {
        return {
            steps: [
                {
                    id: 'fallback_search',
...
exports.IntentParsingService = IntentParsingService;
// Factory function to create IntentParsingService with default config
function createIntentParsingService(amazonQService) {
...
//# sourceMappingURL=IntentParsingService.js.map