...
exports.PIIRedactorClass = exports.getPIIRedactor = exports.PIIRedactorService = void 0;
...
class PIIRedactorService {
    constructor() {
        ...
        this.kms = new aws_sdk_1.KMS({ region: process.env.AWS_REGION || 'us-east-1' });
        this.dynamodb = new aws_sdk_1.DynamoDB({ region: process.env.AWS_REGION || 'us-east-1' });
    ...
    redactQuery(query) {
        ...
        this.piiPatterns.forEach((pattern, index) => {
            sanitizedText = sanitizedText.replace(pattern, (match) => {
                const token = `[PII_TOKEN_${index}_${(0, uuid_1.v4)().substring(0, 8)}]`;
        ...
        const processObject = (obj, prefix = '') => {
            ...
            for (const [key, value] of Object.entries(obj)) {
                ...
                if (this.sensitiveFields.some(field => field.toLowerCase() === key.toLowerCase()) && typeof value === 'string') {
                    const token = `[USER_TOKEN_${(0, uuid_1.v4)().substring(0, 8)}]`;
    ...
    detokenize(tokenizedText, tokenMap) {
     ...
     */
    async createSecureToken(value, dataType, merchantId, userId, expiresInHours) {
        try {
            // Generate unique token ID
            const tokenId = `${dataType}_${(0, uuid_1.v4)().replace(/-/g, '')}`;
     ...
     */
    async retrieveFromToken(tokenId, merchantId) {
        try {
     ...
     */
    async tokenizePaymentData(paymentData, merchantId, userId) {
        const tokenizedData = {};
        const tokenMappings = [];
        for (const [field, value] of Object.entries(paymentData)) {
            if (value && this.paymentFields.includes(field)) {
                try {
                    const token = await this.createSecureToken(typeof value === 'string' ? value : JSON.stringify(value), 'payment', merchantId, userId, 24 // Payment tokens expire in 24 hours
                    );
     ...
     */
    async sanitizeConversationLog(conversationData, merchantId) {
     ...
     */
    async sanitizeObject(obj, merchantId) {
     ...
     * Store token mapping in DynamoDB
     */
    async storeTokenMapping(tokenRecord) {
        const item = {
     ...
     * Get token mapping from DynamoDB
     */
    async getTokenMapping(tokenId, merchantId) {
        try {
            ...
            return {
                ...
                data_type: result.Item.data_type.S,
     ...
     */
    async deleteTokenMapping(tokenId, merchantId) {
        try {
     ...
     */
    async cleanupExpiredTokens(merchantId) {
...
exports.PIIRedactorService = PIIRedactorService;
// Export singleton instance
let piiRedactorInstance = null;
const getPIIRedactor = () => {
    if (!piiRedactorInstance) {
        piiRedactorInstance = new PIIRedactorService();
    }
    return piiRedactorInstance;
};
exports.getPIIRedactor = getPIIRedactor;
// Export class alias to avoid conflict
exports.PIIRedactorClass = PIIRedactorService;
