...
const PredictorDeploymentService_1 = require("../services/PredictorDeploymentService");
const MindsDBService_1 = require("../services/MindsDBService");
const CacheService_1 = require("../services/CacheService");
// Initialize services
let semanticRetrievalService;
let predictorDeploymentService;
let mindsdbService;
let cacheService;
...
async function initializeServices() {
    if (!isInitialized) {
        ...
        predictorDeploymentService = new PredictorDeploymentService_1.PredictorDeploymentService(mindsdbService);
 ...
 */
async function handleModelDeployment(event, context) {
    ...
    try {
        ...
        if (deploymentRequest.modelType === 'embedding' || deploymentRequest.modelType === 'both') {
            ...
            try {
                const embeddingDeployment = await predictorDeploymentService.redeployPredictor(`embedding_model_${deploymentRequest.merchantId}`, {
        ...
        if (deploymentRequest.modelType === 'reranking' || deploymentRequest.modelType === 'both') {
            ...
            try {
                const rerankingDeployment = await predictorDeploymentService.redeployPredictor(`reranking_model_${deploymentRequest.merchantId}`, {
                    merchantId: deploymentRequest.merchantId,
                    trainingDataTable: `documents_${deploymentRequest.merchantId}`,
                    features: {
                        demographic: [],
        ...
        const overallStatus = Object.values(results).every((result) => result.status === 'deployed' || result.status === 'configured')
 ...
 */
async function handleSemanticSearch(event, context) {
    const body = JSON.parse(event.body || '{}');
    const searchRequest = {
        query: body.query,
        merchantId: body.merchantId,
 ...
 */
async function handleRestSearch(event, context) {
    const body = JSON.parse(event.body || '{}');
    const restSearchRequest = {
        query: body.query,
        merchantId: body.merchantId,
 ...
 */
async function handleGroundingValidation(event, context) {
    const body = JSON.parse(event.body || '{}');
    ...
    try {
        ...
        const validationResults = await Promise.all(validationRequest.documents.map(async (doc, index) => {
            try {
 ...
 */
async function handleDeploymentStatus(merchantId, context) {
    ...
    try {
        ...
        const merchantPredictors = predictors.filter((p) => p.name.includes(merchantId) && (p.name.includes('embedding') || p.name.includes('reranking')));
        ...
        const status = {
            ...
            embedding_model: {
                status: merchantPredictors.find((p) => p.name.includes('embedding')) ? 'deployed' : 'not_deployed',
                predictor_name: `embedding_model_${merchantId}`
            },
            reranking_model: {
                status: merchantPredictors.find((p) => p.name.includes('reranking')) ? 'deployed' : 'not_deployed',
 ...
 */
async function handleUpdateConfig(merchantId, event, context) {
    const body = JSON.parse(event.body || '{}');
