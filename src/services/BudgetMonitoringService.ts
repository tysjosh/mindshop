import { BudgetsClient, CreateBudgetCommand, CreateNotificationCommand, DescribeBudgetsCommand } from '@aws-sdk/client-budgets';
...
export class BudgetMonitoringService {
  ...
  constructor(accountId: string, region: string = 'us-east-1') {
    this.budgets = new BudgetsClient({ region });
    this.accountId = accountId;
    this.setupDefaultBudgets();
  }

  private async setupDefaultBudgets(): Promise<void> {
  ...
  public async createBudget(config: BudgetConfig): Promise<void> {
    const context: LogContext = {
      merchantId: 'system',
  ...
  private formatCostFilters(filters?: BudgetConfig['costFilters']): any {
  ...
  private async createBudgetNotification(
    ...
    try {
      const notificationCommand = new CreateNotificationCommand({
        AccountId: this.accountId,
  ...
  public async checkBudgetStatus(): Promise<CostAlert[]> {
    const context: LogContext = {
      merchantId: 'system',
    ...
    try {
      const response = await this.budgets.send(new DescribeBudgetsCommand({
        AccountId: this.accountId
      }));
      ...
      await this.loggingService.logInfo(
        'Budget status check completed',
        context,
        { budgetsChecked: response.Budgets?.length || 0, alertsTriggered: alerts.length }
      );
  ...
  private extractServicesFromFilters(costFilters?: any): string[] {
    if (!costFilters || !costFilters.Service) return [];
  ...
  public async getCostPerSession(merchantId: string, timeRange: { start: Date; end: Date }): Promise<number> {
    ...
    try {
      ...
      await this.loggingService.logInfo(
        ...
        context,
        { costPerSession: estimatedCostPerSession }
      );
  ...
  public async setupCostOptimizationAlerts(): Promise<void> {
    const context: LogContext = {
      merchantId: 'system',
...
// Singleton instance
let budgetMonitoringInstance: BudgetMonitoringService | null = null;

export function getBudgetMonitoringService(): BudgetMonitoringService {
  if (!budgetMonitoringInstance) {
    ...
    budgetMonitoringInstance = new BudgetMonitoringService(
      accountId,
