...
import { TransactionRepository, TransactionState, CompensationAction } from '../repositories/TransactionRepository';
...
export class CompensationService {
  ...
  async executeCompensation(
    ...
    try {
      ...
      for (const actionConfig of compensationActions) {
        try {
          const actionResult = await this.executeCompensationAction(
            transaction,
            actionConfig.type as CompensationAction['action_type'],
            failureReason
          );
  ...
  private async executeCompensationAction(
    ...
    actionType: CompensationAction['action_type'],
    reason: string
    ...
    const action = await this.transactionRepository.addCompensationAction(
    ...
    try {
      ...
      await this.transactionRepository.updateCompensationAction(
        transaction.transaction_id,
        transaction.merchant_id,
        action.action_id,
        {
   ...
   */
  private async releaseInventory(transaction: TransactionState): Promise<{ success: boolean; error?: string }> {
   ...
   */
  private async processRefund(transaction: TransactionState, reason: string): Promise<{ success: boolean; error?: string }> {
   ...
   */
  private async cancelOrder(transaction: TransactionState, reason: string): Promise<{ success: boolean; error?: string }> {
    try {
   ...
   */
  private async sendNotifications(transaction: TransactionState, reason: string): Promise<{ success: boolean; error?: string }> {
    try {
   ...
   */
  async retryFailedCompensations(merchantId?: string): Promise<{
    retried: number;
    succeeded: number;
    failed: number;
  }> {
    ...
    for (const action of pendingActions) {
      ...
      try {
        ...
        const transaction = await this.transactionRepository.getTransaction(
          action.metadata.transaction_id,
          merchantId || action.metadata.merchant_id
        );
