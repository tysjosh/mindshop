...
export class ConversationSanitizationService {
  private piiRedactor: PIIRedactorService;
  private auditLogRepository: AuditLogRepository;

  constructor(
    piiRedactor: PIIRedactorService,
   ...
   */
  async sanitizeConversationEntry(
    conversation: ConversationEntry
  ): Promise<SanitizedConversationEntry> {
    ...
    try {
      ...
      if (hasTransactionRefs) {
        sanitizedResponse = await this.sanitizeTransactionReferences(
          sanitizedResponse,
          conversation.transaction_references!,
          conversation.merchant_id
        );
      }
      ...
      const sanitizedEntry: SanitizedConversationEntry = {
   ...
   */
  private async sanitizeTransactionReferences(
    response: string,
    ...
    for (const transactionRef of transactionRefs) {
      try {
        ...
        const token = await this.piiRedactor.createSecureToken(
          transactionRef,
          'payment',
          merchantId,
          undefined,
          72 // Transaction references expire in 72 hours
        );
   ...
   */
  async validateNoPaymentTokens(conversationData: Record<string, any>): Promise<{
    is_clean: boolean;
    violations: string[];
   ...
   */
  private async removePaymentTokens(data: Record<string, any>): Promise<Record<string, any>> {
   ...
   */
  private createFallbackSanitizedEntry(conversation: ConversationEntry): SanitizedConversationEntry {
    return {
      session_id: conversation.session_id,
      merchant_id: conversation.merchant_id,
      user_id: conversation.user_id,
   ...
   */
  private hashConversation(conversation: ConversationEntry): string {
   ...
   */
  async batchSanitizeConversations(
    ...
    const sanitized: SanitizedConversationEntry[] = [];
