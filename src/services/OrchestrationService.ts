...
import { getBedrockAgentService, BedrockAgentService } from './BedrockAgentService';
import { createSessionManager, SessionManager } from './SessionManager';
import { getSessionAnalyticsService, SessionAnalyticsService } from './SessionAnalyticsService';
...
export class OrchestrationService {
  ...
  private readonly LATENCY_BUDGET_MS = 300;
  
  // Cost tracking
  private readonly COST_PER_REQUEST_BASE = 0.001; // $0.001 base cost per request

  constructor() {
    this.ragService = new RAGService();
    this.bedrockAgentService = getBedrockAgentService();
    ...
    this.checkoutService = new CheckoutService(this.auditLogRepository, this.piiRedactor);
  }
  ...
  async orchestrateRequest(request: OrchestrationRequest): Promise<OrchestrationResponse> {
    ...
    const componentLatencies: OrchestrationResponse['componentLatencies'] = {
      authentication: 0,
    };
  ...
  private async handleChatRequest(
    ...
    latencies: OrchestrationResponse['componentLatencies']
  ): Promise<any> {
    ...
    const ragResult = await this.circuitBreaker.callWithBreaker(
      async () => {
    ...
    const agentResponse = await this.circuitBreaker.callWithBreaker(
      async () => {
    ...
    return {
      data: {
        ...
        sources: ragResult.retrievalResults.map((r: any) => ({
          id: r.id,
  ...
  private async handleAnalyticsRequest(
    ...
    if (type === 'billing') {
      analyticsResult = await this.sessionAnalyticsService.generateBillingData(
        request.merchantId,
        new Date(startDate),
        new Date(endDate)
      );
    } else {
      analyticsResult = await this.sessionAnalyticsService.getSessionAnalytics(
        request.merchantId,
        new Date(startDate),
        new Date(endDate)
      );
  ...
  async performHealthCheck(): Promise<HealthCheckResult> {
    ...
    } as HealthCheckResult['components'];
  ...
  private async sanitizeRequest(request: OrchestrationRequest): Promise<OrchestrationRequest> {
  ...
  private calculateChatCost(ragResult: any, agentResponse: any): number {
  ...
  private calculateSearchCost(ragResult: any): number {
    let cost = 0.0005; // Base search cost
    cost += ragResult.retrievalResults.length * 0.0001; // Cost per result
    return cost;
  }

  private calculateCheckoutCost(checkoutResult: any): number {
    return 0.01; // Fixed cost for checkout processing
  }

  private async testSessionManager(): Promise<{ status: string }> {
  ...
  private async testCheckoutService(): Promise<{ status: string }> {
    // Simple health check for checkout service
    return { status: 'healthy' };
  }

  private getHealthStatus(result: PromiseSettledResult<any>): 'healthy' | 'degraded' | 'unhealthy' {
...
export const getOrchestrationService = (): OrchestrationService => {
