 ...
 */
export class PredictorDeploymentService {
   ...
   */
  async deployProductSignalsPredictor(config: DeploymentConfig): Promise<DeploymentStatus> {
    ...
    try {
      ...
      const predictorConfig: ProductSignalsPredictorConfig = {
        merchantId: config.merchantId,
   ...
   */
  async listMerchantDeployments(merchantId: string): Promise<DeploymentStatus[]> {
    try {
      const result = await this.mindsdbService.query('SHOW MODELS');
      const predictors = result.data.filter((model: any) => 
        model.name && model.name.includes(merchantId)
      );
      
      return predictors.map((predictor: any) => ({
        merchantId,
   ...
   */
  private async createProductSignalsPredictor(config: ProductSignalsPredictorConfig): Promise<void> {
    const predictorName = `product_signals_${config.merchantId}`;
   ...
   */
  private async validateTrainingData(tableName: string, merchantId: string): Promise<void> {
    const validationSQL = `
   ...
   */
  private async validatePredictorFunctionality(predictorName: string, merchantId: string): Promise<void> {
    try {
...
let deploymentServiceInstance: PredictorDeploymentService | null = null;

export const getPredictorDeploymentService = (): PredictorDeploymentService => {
