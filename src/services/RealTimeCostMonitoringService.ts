...
export class RealTimeCostMonitoringService {
  ...
  private activeScalingActions: Set<string> = new Set();
  ...
  private async performCostCheck(): Promise<void> {
    ...
    try {
      ...
      await this.loggingService.logInfo(
        'Cost monitoring check completed',
        context,
        {
  ...
  private async getCurrentCostMetrics(): Promise<RealTimeCostMetrics> {
  ...
  private async checkThreshold(threshold: CostThreshold, metrics: RealTimeCostMetrics): Promise<void> {
    let currentValue = 0;
  ...
  private async handleThresholdBreach(
    ...
    await this.loggingService.logWarning(
      `Cost threshold breached: ${threshold.name}`,
      context,
      {
  ...
  private async executeScalingAction(actionType: string): Promise<void> {
    ...
    if (!action) {
      await this.loggingService.logWarning(
        `Scaling action not found: ${actionType}`,
        {
          merchantId: 'system',
    ...
    try {
      ...
      await this.loggingService.logInfo(
        `Scaling action executed: ${actionType}`,
        context,
        {
          actionType,
      ...
      await this.loggingService.logError(error as Error, context, { actionType });
    }
  }
  ...
  private async enterEmergencyMode(): Promise<void> {
    ...
    await this.loggingService.logError(
      new Error('Emergency cost threshold reached - entering emergency mode'),
      context
    );
    ...
    // Send critical alert
    const emergencyAlert: AlertNotification = {
  ...
  private async emitCostMetrics(metrics: RealTimeCostMetrics): Promise<void> {
  ...
  public async getCostProjection(timeHorizonHours: number): Promise<number> {
    const currentMetrics = await this.getCurrentCostMetrics();
    return currentMetrics.currentHourlyCost * timeHorizonHours;
  }

  public async reverseScalingAction(actionType: string): Promise<void> {
    if (!this.activeScalingActions.has(actionType)) return;
    ...
    await this.loggingService.logInfo(
      `Scaling action reversed: ${actionType}`,
      {
        merchantId: 'system',
...
// Factory function
export function createRealTimeCostMonitoringService(config?: Partial<CostMonitoringConfig>): RealTimeCostMonitoringService {
  const defaultConfig: CostMonitoringConfig = {
