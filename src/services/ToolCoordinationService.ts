...
export class ToolCoordinationService {
  ...
  private bulkheads: Map<string, MerchantBulkhead> = new Map();
  ...
  registerTool(tool: ToolDefinition): void {
    ...
    this.circuitBreakers.set(tool.id, new CircuitBreakerService());
  ...
  async invokeTool(invocation: ToolInvocation): Promise<ToolResult> {
    ...
    try {
      ...
      const result = await circuitBreaker.callWithBreaker(
        () => this.executeToolCall(tool, invocation),
        () => this.getFallbackResult(invocation),
        tool.circuitBreakerConfig
      );
    ...
    try {
      if (plan.parallelizable) {
        ...
        const parallelResults = await Promise.allSettled(
          invocations.map(inv => this.invokeTool(inv))
        );
  ...
  getSystemHealth(): {
    ...
    const toolsHealth: Record<string, any> = {};
    ...
    const bulkheadStats: Record<string, BulkheadStats> = {};
   ...
   */
  private async executeToolCall(tool: ToolDefinition, invocation: ToolInvocation): Promise<any> {
   ...
   */
  private async getFallbackResult(invocation: ToolInvocation): Promise<any> {
   ...
   */
  private getBulkhead(merchantId: string): MerchantBulkhead {
   ...
   */
  private startHealthCheck(tool: ToolDefinition): void {
   ...
   */
  private topologicalSort(steps: ExecutionPlan['steps']): ExecutionPlan['steps'] {
...
class MerchantBulkhead {
  ...
  constructor(merchantId: string, config: { maxConcurrentRequests: number; queueSize: number }) {
  ...
  releaseSlot(success: boolean): void {
...
export function createToolCoordinationService(): ToolCoordinationService {
